...
export const PickupScreen = ({ route }: PickupScreenProps) => {
  ...
  const styles = StyleSheet.create({
    ...
    packageInfo: { marginTop: spacing.xl },
    infoGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md },
    infoCard: { flex: 1, minWidth: '45%', padding: 16, gap: 10 },
    infoIcon: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' },
    infoLabel: { fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, fontSize: fonts.size.xs },
  ...
  const InfoItem = ({ label, value, icon, color }: { label: string; value: string; icon: ComponentProps<typeof Ionicons>['name']; color: string }) => {
    return (
      <View style={[styles.infoCard, { backgroundColor: colors.surface, borderRadius: radii.lg, ...shadows.sm }]}>
        ...
        <View>
          ...
          <Text style={[styles.infoValue, { color: colors.textPrimary, fontSize: fonts.size.sm }]}>{value}</Text>
        </View>
      </View>
    );
  };
  ...
  const verifyOTP = async () => {
    ...
    setVerifying(true);
    try {
      await api.post(`${ENDPOINTS.driver}/orders/${orderId}/verify-pickup`, { otp }, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      setPickupConfirmed(true);
      // 'pickup' is the real OrderStatus enum value — 'picked_up' isn't.
      await api.patch(ENDPOINTS.orders.updateStatus(orderId), { status: 'pickup' }, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
        <View style={styles.header}>
          <Header title="Pickup Order" leftAction={{ icon: 'chevron-back', onPress: goBack }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.orderCard}>
            ...
            <View style={styles.addressRow}>
              ...
              <Text style={[styles.address, { color: colors.textPrimary }]}>{order.pickupAddress}</Text>
            </View>
          </View>
          ...
          {pickupConfirmed ? (
            <View style={styles.successCard}>
              ...
              <Text style={[styles.successTitle, { color: colors.textPrimary }]}>Order Picked Up!</Text>
              <Text style={[styles.successDesc, { color: colors.textSecondary }]}>The package has been collected. You can now navigate to the delivery location.</Text>
              <ActionButton
                label="Start Navigation to Delivery"
                icon="navigate"
                variant="primary"
                size="lg"
                fullWidth
                onPress={() => navigate('Navigation', { orderId })}
              />
            </View>
          ) : (
            <>
              <View style={styles.otpSection}>
                ...
                <ActionButton
                  label={verifying ? 'Verifying...' : 'Verify & Pickup'}
              ...
              <View style={styles.packageInfo}>
                ...
                <View style={styles.infoGrid}>
                  <InfoItem label="Weight" value={order.weight ? `${order.weight} kg` : 'N/A'} icon="scale-outline" color="#F97316" />
                  <InfoItem label="Dimensions" value={order.dimensions || 'N/A'} icon="resize-outline" color="#06B6D4" />
                  <InfoItem label="Priority" value={order.priority} icon="flash-outline" color="#F59E0B" />
                  <InfoItem label="Distance" value={`${order.distance} km`} icon="navigate-outline" color="#8B5CF6" />
                </View>
              </View>
            </>
          )}
