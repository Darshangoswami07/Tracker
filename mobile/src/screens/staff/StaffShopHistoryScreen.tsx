      ...
      >
        <View style={styles.rowTop}>
          ...
          <View style={[styles.badge, { backgroundColor: `${statusColor}18` }]}>
            <Text style={[styles.badgeText, { color: statusColor }]} numberOfLines={1}>{t(`summary.${item.status}`, item.status)}</Text>
          </View>
        </View>
...
const createStyles = (theme: Pick<AppTheme, 'colors' | 'spacing' | 'radii' | 'fonts' | 'shadows'>) =>
  StyleSheet.create({
    ...
    // unbroken long word overflows the card horizontally on web instead of
