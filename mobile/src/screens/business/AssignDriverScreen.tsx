...
export const AssignDriverScreen = () => {
  const { colors, spacing, radii, fonts, shadows } = useAppTheme();
  const { goBack, navigate, role } = useAppNav();
  const accessToken = useAuthStore((state) => state.accessToken);
  const isEmployee = role === 'employee' || role === 'dispatcher';
  const ordersBase = isEmployee ? ENDPOINTS.employee : ENDPOINTS.business;
  ...
  const handleAdd = () => navigate('CreateOrder');
  const openOrder = (order: Order) => navigate('OrderDetails', { orderId: order.id });
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
        <View style={styles.header}>
          <Header title="Orders" leftAction={{ icon: 'chevron-back', onPress: goBack }} rightAction={{ icon: 'add', onPress: handleAdd }} />
        </View>
        ...
        {orders.length === 0 ? (
          ...
          <>
            <View style={styles.ordersList}>
              {orders.map((order) => (
                <TouchableOpacity key={order.id} style={styles.orderCard} onPress={() => openOrder(order)} activeOpacity={0.85}>
                  ...
                  <View style={styles.orderFooter}>
                    <View style={styles.orderMeta}>
                      <Text style={{ color: colors.textMuted, fontSize: fonts.size.xs }}>{order.customerName}</Text>
                      <Text style={{ color: colors.textMuted, fontSize: fonts.size.xs }}>{new Date(order.createdAt).toLocaleDateString()}</Text>
                    </View>
                    <View style={styles.orderRight}>
                      <Text style={[styles.orderAmount, { color: colors.textPrimary }]}>₹{order.amount.toLocaleString()}</Text>
                      {order.driverName && (
