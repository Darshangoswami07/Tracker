...
export const BusinessReportsScreen = () => {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      const res = await api.get(`${ENDPOINTS.business}/reports`, {
  ...
  const generateReport = async (typeId: string) => {
    try {
      await api.post(`${ENDPOINTS.business}/reports/generate`, { type: typeId }, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
        ...
        >
          ...
          {filteredReports.length === 0 ? (
            <EmptyState
              ...
              onActionPress={() => filter !== 'all' ? setFilter('all') : generateReport('orders')}
              iconColor="#635BFF"
            />
          ) : (
            <View style={styles.reportsList}>
              {filteredReports.map((report) => (
                <TouchableOpacity key={report.id} style={styles.reportCard} onPress={() => report.downloadUrl && Linking.openURL(report.downloadUrl).catch(() => {})} activeOpacity={0.85}>
                  ...
                  <View style={styles.reportRight}>
                    ...
                    {report.downloadUrl && (
                      <TouchableOpacity style={styles.downloadBtn} onPress={() => report.downloadUrl && Linking.openURL(report.downloadUrl).catch(() => {})}>
...
const createStyles = (theme: Pick<AppTheme, 'colors' | 'spacing' | 'radii' | 'fonts' | 'shadows'>) =>
  StyleSheet.create({
    ...
    typesGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.md },
    typeCard: { flex: 1, minWidth: '45%', padding: 16, alignItems: 'flex-start', gap: theme.spacing.sm },
    typeIcon: { width: 56, height: 56, alignItems: 'center', justifyContent: 'center' },
    typeName: { fontSize: theme.fonts.size.md, fontWeight: '800' },
    typeDesc: { fontSize: theme.fonts.size.sm, fontWeight: '500' },
    typeCardShimmer: { marginBottom: theme.spacing.md, borderRadius: theme.radii.xl },
    sectionTitleShimmer: { width: 150, height: 24, borderRadius: theme.radii.sm },
    reportsList: { gap: theme.spacing.md },
    reportCard: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, backgroundColor: theme.colors.surface, borderRadius: theme.radii.lg, ...theme.shadows.sm },
    ...
    reportInfo: { gap: 4 },
    reportName: { fontSize: theme.fonts.size.md, fontWeight: '700' },
    reportMeta: { flexDirection: 'row', gap: 12 },
    reportRight: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    downloadBtn: { width: 40, height: 40, borderRadius: theme.radii.md, alignItems: 'center', justifyContent: 'center', backgroundColor: '#635BFF15' },
    reportCardShimmer: { marginBottom: theme.spacing.md, borderRadius: theme.radii.lg },
  });
